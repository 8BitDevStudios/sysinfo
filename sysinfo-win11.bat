...
for /f "usebackq" %%a in (`powershell -command "[math]::Floor((Get-CimInstance Win32_ComputerSystem).TotalPhysicalMemory / 1GB)"`) do set ram_gb=%%a
for /f "usebackq" %%a in (`powershell -command "[math]::Floor((Get-CimInstance Win32_VideoController | Select-Object -First 1).AdapterRAM / 1GB)"`) do set vram_gb=%%a
